...
import {Protect, useClerk, UserAvatar, useUser} from '@clerk/clerk-react'
import { Eraser, FileText, Hash, House, Icon, Image, LogOut, Scissors, SquarePen, User, Users } from 'lucide-react'
...
const navItems = [
    ...
    {to: '/ai/remove-backgroung', label: 'Remove Background', Icon : Eraser},
            ...
            <div className='px-6 mt-5 text-sm text-gray-600 font-medium'>
                {
                    navItems.map(({to, label, Icon})=>(
                        <NavLink key={to} to={to} end={to === '/ai'} onClick={()=>
                            setSidebar(false)} className={({isActive})=> `px-3.5 py-2.5 flex items-center gap-3 rounded ${isActive ? `bg-gradient-to-r from-[crimson] to-[#9234EA] text-white` : ''}`}>
                                {({isActive})=>(
                                   <>
                ...
                <div onClick={openUserProfile} className='flex gap-2 items-center cursor-pointer'>
                    ...
                    <div>
                        ...
                        <p className='text-xs text-gray-500'>
                            <Protect plan='premium' fallback=''>Premium</Protect>
                        </p>
                    </div>
